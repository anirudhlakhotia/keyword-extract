...
def extract_info(attributes):
    ...
    love_value = attributes_json.get('comment_answers', {}).get('love', {}).get('value', '')
    hate_value = attributes_json.get('comment_answers', {}).get('hate', {}).get('value', '')
...
def read_csv_and_extract(filename):
    ...
    with open(filename, 'r', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            ...
            product_name, star_rating, title, love_value, hate_value = extract_info(attributes)
...
def write_extracted_csv(reviews, filename):
    with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['id', 'product_name', 'star_rating', 'title', 'love_value', 'hate_value']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
